...
REF_PATH = sys.argv[1]
INPUT_TESTSET_FILE_PATH = "input.in"
OUTPUT_TESTSET_FILE_PATH = "output.out"
MAX_NUM_TESTS = 100

scanner_functions = ['{VarName}.nextInt()','{VarName}.nextLong()','{VarName}.nextFloat()','{VarName}.nextDouble()', '{VarName}.nextShort()','{VarName}.next()']
...
def computeIterations(loop):
	operators = ['<','>','>=','<=']
	line_split = loop.split(";")
	initial_value = int(line_split[0].split('=')[1])
	final_value = 0	
	for op in operators:
		if(op in line_split[1]):
			final_value = int(line_split[1].split(op)[1])
...
with open("Reference/"+REF_PATH,'r') as fin:
	...
	for line in lines:
		...
		if(var_name!=""):
			...
			if is_assert == False:			
				if("for" in line):	
					iterations = computeIterations(line)
